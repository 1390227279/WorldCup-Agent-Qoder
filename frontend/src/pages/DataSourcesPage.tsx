...
export default function DataSourcesPage() {
  ...
  return <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6">
    ...
    <section className="dashboard-card mb-5 p-5">
      <div className="flex flex-wrap items-start justify-between gap-4"><div><h2 className="text-lg font-semibold text-white">真实网络采集</h2><p className="mt-1 text-sm text-[var(--color-text-muted)]">只能调用后端白名单中的数据源，不能输入任意 URL。</p></div><button type="button" onClick={() => void refreshRuns()} className="rounded-md border border-[var(--color-border)] px-3 py-2 text-sm text-[var(--color-text-muted)] hover:text-white">刷新账本</button></div>
    ...
    <section className="dashboard-card mb-5 overflow-hidden">
      ...
      <div className="overflow-x-auto"><table className="w-full min-w-[1050px] text-sm"><thead className="bg-[var(--color-surface-raised)] text-xs text-[var(--color-text-muted)]"><tr><th className="px-4 py-3 text-left">运行</th><th className="px-3 py-3 text-left">来源</th><th className="px-3 py-3">状态</th><th className="px-3 py-3">HTTP</th><th className="px-3 py-3">快照</th><th className="px-3 py-3">原始/更新/跳过</th><th className="px-3 py-3 text-left">SHA-256</th><th className="px-4 py-3">操作</th></tr></thead><tbody>
        {runs.data?.map((run) => <tr key={run.id} className="border-t border-[var(--color-border-muted)]"><td className="px-4 py-3"><b className="font-mono text-white">#{run.id}</b><p className="mt-1 text-xs text-[var(--color-text-muted)]">{new Date(run.started_at).toLocaleString("zh-CN")}</p></td><td className="px-3 py-3 text-white">{run.source_name}</td><td className="px-3 py-3 text-center"><span className={`rounded-full border px-2 py-1 text-xs ${statusClasses[run.status]}`}>{statusLabels[run.status]}</span></td><td className="px-3 py-3 text-center font-mono">{run.http_status ?? "—"}</td><td className="px-3 py-3 text-center">{formatBytes(run.snapshot_bytes)}</td><td className="px-3 py-3 text-center font-mono">{run.raw_record_count} / {run.updated_team_count} / {run.skipped_team_count}</td><td className="max-w-52 truncate px-3 py-3 font-mono text-xs text-[var(--color-text-muted)]" title={run.sha256_hash ?? ""}>{run.sha256_hash ?? "—"}</td><td className="px-4 py-3 text-center">{run.status === "FETCHED" && run.source_name === "world_football_elo" ? <button type="button" disabled={activeAction !== null} onClick={() => void processRun(run)} className="rounded-md border border-[var(--color-primary)] px-3 py-1.5 text-xs font-semibold text-[var(--color-primary)] disabled:opacity-50">{activeAction === `process-${run.id}` ? "处理中……" : "解析并更新球队"}</button> : run.status === "FETCHED" ? <span className="text-xs text-[var(--color-text-muted)]">对应加载器待接入</span> : <span className="text-xs text-[var(--color-text-muted)]">—</span>}</td></tr>)}
