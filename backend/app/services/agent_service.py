...
输入中的 tournament_name、tournament_year 和 tournament_code 是本报告唯一有效的赛事时间锚点。
必须明确围绕该届赛事撰写，禁止自行推断赛季或把报告写成其他年份的世界杯。
只有输入 JSON 明确出现的年份才可以写入报告；不得根据模型记忆补充年份、赛季或时间线。
...
class AgentService:
    ...
    async def analyze_tournament(self, context: dict) -> dict:
        tournament_name = context.get("tournament_name", "当前世界杯")
        tournament_year = context.get("tournament_year", "未知年份")
        messages = [
            {"role": "system", "content": TOURNAMENT_REPORT_PROMPT},
            {
                "role": "user",
                "content": (
                    f"请解释 {tournament_year} 年的 {tournament_name} 冠军推演。"
                    "不得写成其他年份的赛事。\n"
                    + json.dumps(context, ensure_ascii=False)
                ),
            },
        ]
        submit_tool = self._submit_tournament_report_tool_def()
        report = await self._request_tournament_report(messages, submit_tool)
        unsupported_years = self._unsupported_report_years(report, context)
        if unsupported_years:
            correction_messages = [
                {"role": "system", "content": TOURNAMENT_REPORT_PROMPT},
                {
                    "role": "user",
                    "content": (
                        f"上一版错误使用了上下文中不存在的年份：{', '.join(unsupported_years)}。"
                        f"请丢弃上一版，只围绕 {tournament_year} 年的 {tournament_name}，"
                        "立即调用 submit_tournament_report 重新提交。权威上下文如下：\n"
                        + json.dumps(context, ensure_ascii=False)
                    ),
                },
            ]
            report = await self._request_tournament_report(correction_messages, submit_tool)
            unsupported_years = self._unsupported_report_years(report, context)
        if not isinstance(report, dict):
            raise ValueError("Qwen 未提交冠军推演报告")
        if unsupported_years:
            raise ValueError(f"冠军推演报告包含无依据年份：{', '.join(unsupported_years)}")
    ...
    async def _request_tournament_report(self, messages: list[dict], submit_tool: dict) -> Optional[dict]:
        response = await self.qwen_client.chat_with_tools(messages, [submit_tool])
        report = next(
            (
                call.arguments
                for call in response.tool_calls
                if call.name == "submit_tournament_report"
            ),
            None,
        )
    ...
    @staticmethod
    def _unsupported_report_years(report: Optional[dict], context: dict) -> list[str]:
        if not isinstance(report, dict):
            return []
        year_pattern = r"(?<!\d)(?:19|20)\d{2}(?!\d)"
        allowed_years = set(re.findall(year_pattern, json.dumps(context, ensure_ascii=False)))
        report_years = set(re.findall(year_pattern, json.dumps(report, ensure_ascii=False)))
        return sorted(report_years - allowed_years)
