...
- 数学影响事件：在泊松抽样前修改本次情景的期望进球；
- 叙事背景事件：只进入相关单场比赛的 Qwen 解读上下文，不修改比分和概率。
...
修正值按比例合并并限制在安全范围内。没有有效 lambda、但具备标题或描述背景的事件进入 `narrative_events`，不再被视为失败。停用、未生效、已过期、不属于当前赛事或格式错误的事件才进入 `ignored_events`。旧 `attack/defense` 字段只在事件兼容解析层转换，标准接口和管理页面使用新字段。

当选择的事件全部是叙事背景时，后端复制基线的 `model/summary/representative_path`，仅生成新的轻量上下文和 `simulation_id`，不会再次运行蒙特卡洛模拟。混合选择时，只有数学事件进入模拟，叙事事件随结果保存供单场分析使用。
...
- `math`：比分、胜者、晋级方式、主客队期望进球、胜平负/晋级概率、数学影响事件和叙事背景事件；
- `agent`：可选的 Qwen 解释报告。
