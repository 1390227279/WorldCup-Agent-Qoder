...
## 不可违反的约束
- 输入中的球队、比分、胜者、胜率、预期进球 lambda 和事件均为后端数学事实。
- 你不得重新预测、改写、纠正或否定这些数学事实。
- 你只能解释这些数据可能反映的战术原因、关键因素和不确定性。
- 不得自行补充未出现在“已应用事件”中的伤病、换帅或其他事件。
- 若调用外部分析工具得到不同比分或胜者，仍必须以输入中的数学事实为准。

## 工作目标
1. 结合 ELO、排名、近期状态和历史交锋解释既有数学结果。
2. 说明已应用事件如何影响对应球队的 lambda。
3. 给出 3-5 个关键因素、风险提示和连续编号的推理步骤。
4. 最后必须调用 submit_match_analysis；该工具不允许提交比分、胜者或概率。
"""
...
class AgentService:
    ...
    async def analyze_simulated_match(
        ...
        """解释模拟比赛的既有数学结论，不允许 Agent 改写结果。"""
        home_team = match_context["home_team"]["name"]
        away_team = match_context["away_team"]["name"]
        context_json = json.dumps(match_context, ensure_ascii=False, indent=2)
        messages = [
            ...
            if response.tool_calls:
                for tool_call in response.tool_calls:
                    ...
                    all_tool_records.append(ToolCallRecord(
                        ...
                        success=not tool_result.startswith("工具执行"),
                    ))
                    messages.extend([
                        {
                            "role": "assistant",
        ...
        if report_data is None:
            return AgentReportValidationResult(
                is_valid=False,
                errors=["Agent 未提交解释报告"],
            )
        report_data["tool_calls_log"] = [
            record.model_dump() for record in all_tool_records
        ]
    ...
    def _submit_match_analysis_tool_def() -> dict:
        return {
            ...
            "function": {
                ...
                "description": "提交对后端数学推演结果的解释，不得提交或修改比分、胜者、概率和 lambda。",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "key_factors": {
                            "type": "array",
                            "items": {"type": "string"},
                            "description": "3-5 个解释既有结果的关键因素",
                        },
                        "risk_notes": {
                            "type": "array",
                            "items": {"type": "string"},
                            "description": "结果不确定性和模型边界提示",
                        },
                        "reasoning_chain": {
                            "type": "array",
                            "items": {
